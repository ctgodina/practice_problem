...
spaced_grouping = 2*3*2*3*3 #to get 7.1 I must space out by 6.1 I 6.1 X 6.1 X


squad = []
not_I = []
for j in range(6): #6 groups dividing the 5 types equally to make up 2916 - 36 
    for i in range(1,6):
        not_I = not_I + [Unit(4, unit_types[i]) for j in range(96)]
...
#7.1X 7.1X
squad = squad + [not_I.pop(0) for x in range(1944)] #2*(2916/3) units make up 2 7.1X 
